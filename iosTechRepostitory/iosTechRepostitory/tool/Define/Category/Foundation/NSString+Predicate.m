...
//  iOS-Category
//
//  Created by Lam BG on 2018/12/10.
...
//    NSString *nicknameRegex = @"^[\u4e00-\u9fa5]{2,8}$";
    NSString *nicknameRegex = @"^([\u4E00-\u9FA5A-Za-z\\s]){2,20}$";
...
- (BOOL) isValidLoginPassword {
    NSString *identifyTest = @"^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,20}$";
    NSPredicate *identifyPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",identifyTest];
    return [identifyPredicate evaluateWithObject:self];
}
...
// 1-64位账号
- (BOOL) isValideAccount {
    NSString *identifyTest = @"^[a-zA-Z][a-zA-Z0-9]{0,63}$";
    NSPredicate *identifyPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",identifyTest];
    return [identifyPredicate evaluateWithObject:self];
}
